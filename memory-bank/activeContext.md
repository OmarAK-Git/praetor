...
**2026-07-30 — Agentic judgment COMPLETE** (05e27cd).
...
4. Build operator-visible features (progressive authorization reporting, exemplars, statute curation) â€” **V2-032â€“V2-036 done**.
5. Defer roadmap-scale features until contracts exist.

## Recently changed

- V2-036: eval regression locking â€” workflow template scenario/waiver discipline; expectation-key CI guard; pytest **132** in evals scope.
- V2-035: statute curation workflow â€” review-only `proposed_statute` artifacts; SOC-lead `promote_statute_curation`; pytest **109** in codification+config scope.
- V2-034: similar-case retrieval â€” human-confirmed precedents ranked and wired via exemplar block; pytest **76** in judgment+annotations scope.
- V2-033: judgment prompt exemplar slot â€” bounded `PromptExemplarBlock`; pytest **9** in prompt isolation tests.
- V2-032: progressive authorization reporting â€” read-only report by target_type/asset_class; pytest **55** in metrics+annotations scope; gate-5 remediation added `_AnnotationBucket` TypedDict + lint fixes.
...
- V2-025: PolicyGate containment boundary â€” AST guard for eligibility helper calls; integration tests prove `account_containment_disabled` cannot be bypassed via direct helper; pytest **128** in contracts+policy scope.
...
- V2-016: static fault-flag guard â€” policy/engine literals âŠ† OutcomeMatrixFaultFlag; edict construction validates flags/SFE; pytest **208** in contracts+policy+evals scope.
...
- Live Splunk HEC demo env-gated â†’ V2-029.
...
4. Intake: DEC-053 â€” `evaluate_policy_gate(..., persist_directive=False)` then one `critical_transaction` for directive + edict co-commit.
5. PolicyGate target selection uses gate-resolved target, not raw bundle re-derivation (AG-0080 â†’ V2-015).
