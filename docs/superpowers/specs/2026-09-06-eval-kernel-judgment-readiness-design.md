...
2. **Judgment proof.** On subject-visible Path A, add cite-to-subject scoring. Compare new vs old and new vs `path_a_fact_count` stump with pre-registered McNemar / stability rules. A clean negative is a successful sprint.
3. **Production readiness.** Only if Sprint 2 earns it. Operator path, live-provider honesty, and merge discipline — not a CBC adapter and not an EventID expansion.
...
| Starting Sprint 3 before Sprint 2 earns it | Production readiness is conditional. Honest stop if `new≈old`. |
...
| Scoring PolicyGate as judgment quality | Gate controls *authority*. Capability scores `ModelJudgment.proposed_disposition` only. |
...
| **2** | Judgment | Subject-visible Path A prompt/bundle view; cite-to-subject join; McNemar + stability write-up vs `old_build` and vs stump | Production cutover; EventID expansion; Path B promotion | Pre-registered primary/secondary/stability read, **or** honest stop (`new≈old` / still ≈ stump) |
| **3** | Production readiness | Operator runbook for live judgment; truncation-vs-parse honesty already in Vertex (PROD-CAP-001) wired into the live opt-in path; merge policy that keeps FakeProvider as the default gate | CBC adapter; envelope expansion; EventID expansion; “we are production-ready” if Sprint 2 did not earn it | Entered **only** if Sprint 2 primary is a real win. Otherwise this sprint does not start. |

Sprint 2 cannot start until Sprint 1 exit is green. Sprint 3 cannot start until Sprint 2’s primary criterion is earned (not merely “we ran the cells”).
...
| `thr.valid_cite_wrong_process` | Citations resolve (`validate_evidence_citations` succeeds) but join to the **wrong** process via `assemble_process_relationships`. Authority must not treat “valid cite” as “right subject.” Sprint 2 cite-to-subject scores this as a model miss if the new arm still proposes contain/escalate on the wrong process. |
...
| `use.demo_honesty_gate` | Demo / walkthrough copy and this kernel’s capability rows do not claim “judgment works” while `cap.stump_parity_guard` is unearned. If demo text asserts capability Sprint 2 has not granted, `failure_class=theater_detector`. |
...
- **Cite-to-subject.** A citation is *resolved* (existing `validate_evidence_citations`) **and** joins to the labeled subject process (GUID / image via the relationship graph). Valid-but-wrong-process cites are scored wrong for capability (`thr.valid_cite_wrong_process` is the unit fixture; the live set applies the same join).
- **Arms.** `old_build` = Sprint 1 Path A bag + current prompt (no cite-to-subject requirement). `new_build` = subject-visible Path A + cite-to-subject scoring. Same anchors, same labels, same org config, same model.
...
- **Provider.** Live Vertex opt-in, spike-honest settings: structured `ModelJudgment`, truncation ≠ parse error. Temperature pinned and recorded before the first call. Three runs per anchor per arm. Majority is the inferential unit.
...
| Arm | Evidence | Scoring extra |
|---|---|---|
| `old_build` | Path A bag as today | Disposition vs frozen label. Citations recorded, not required to join subject. |
| `new_build` | Path A bag + subject graph in excerpts | Disposition vs label **and** cite-to-subject on every scored `escalate` / `auto_contain`. |

Empty-bundle Path A observations are excluded from the judgment score and counted separately (same Guard #3 as the spike). They are correlation findings, not model wins.

### Primary criterion

Pre-registered, in this order:

1. **New vs stump.** McNemar exact two-sided on paired anchor-majority outcomes: `new_build` vs `path_a_fact_count` stump. A claim of capability requires a real win (not p=0.625-class noise). The spike’s stump comparison remains the prior: 22/26 vs 20/26, p=0.625.
2. **New vs old.** McNemar exact two-sided, paired anchor-majority: `new_build` vs `old_build`. This answers “did subject-visible + cite-to-subject change anything,” not “did we beat a straw man.”

**Honest stop (locked):** if `new≈old` under the same absolute-rate epsilon the spike already uses (`AB_TIE_SEPARATION_EPSILON = 0.05`) **or** new is still not distinguishable from the stump, **Sprint 3 does not start.** Write the negative. Do not retune the prompt and rerun as if the experiment were exploratory.

### Secondary (recorded, not used to override a primary miss)

- Cite-to-subject rate on `new_build` (resolved ∩ subject-joined / cited).
- Citation resolution rate (existing validator).
- Benign specificity and malicious recall by arm (cell rates are descriptive; majority + McNemar are inferential).
- Confound report from the spike harness (seed EventID, host, hour) — warn at graded ≥ 0.90. Do not invent new confound features after seeing cells.

### Stability

- Three runs per (anchor, arm). Report unstable anchors separately. Do not sum 1/3 flips into a “miss count.”
- Temperature pinned before first live call; recorded in the artifact header.
...
| Expanding EventIDs or envelope fields “to help the model” | out of protocol; stop |

---

## 7. Sprint 3 — production readiness

**Gate:** Sprint 2 primary is a real win (new distinguishable from stump **and** not a `new≈old` honest stop). If that gate is closed, this section is idle. Do not staff it “anyway.”

### In scope (only if earned)

- Operator runbook: how to run live Vertex on Path A, how to read the scorecard, how truncation is classified, that FakeProvider CI is still the merge gate.
- Live opt-in path uses production `VertexProvider` with PROD-CAP-001 truncation typing. No spike-only provider fork as the production story.
- Document that absolute spike rates still do not transfer to AlertEnvelope traffic. If a follow-on population measurement is ever wanted, it uses **window-pickers** (including CBC ATLASv2 rows as time/host selectors) + existing Path A intake — **not** a CBC type and **not** this sprint’s admission ticket.
- Keep the 15-scenario kernel + Outcome Matrix harness as merge gates. Do not replace them with a notebook.
...
| `unearned_demo_claim` | Demo / walkthrough / kernel copy claims judgment works while stump parity is unearned |
...
| **2 — Judgment** | Artifact + McNemar tables; new beats stump **and** is not a `new≈old` tie; cite-to-subject applied; hard fails absent | `new≈old` or new≈stump → **honest stop, no Sprint 3**; hard fail → do not claim a win |
| **3 — Production readiness** | Runbook + live opt-in honesty; FakeProvider remains merge gate; no adapter | Started without Sprint 2 earn; live Vertex made mandatory CI; CBC adapter or EventID expansion |
...
| Sprint 3 only if Sprint 2’s primary is earned | Staffing Sprint 3 after an honest stop |
