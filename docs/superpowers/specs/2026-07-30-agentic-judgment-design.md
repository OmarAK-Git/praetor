...
| `WiderTelemetryTool` | The alert's own already-correlated `EvidenceBundle` | **Rescoped from the original "wider time window" framing**: `JudgmentRequest` carries no raw telemetry today (correlation already ran before judgment), so a genuine wider-window re-correlation isn't reachable without threading raw Sysmon/Security event lists through the request — a larger orchestrator change deferred as follow-on work. v1 delivers what's immediately reachable and directly answers this spec's original complaint: **untruncated, full-fidelity** re-fetch of facts already in the bundle, replacing the 200-char head-tail truncation entirely for whatever the agent chooses to pull |
