...
| DEC-067 | 2026-08-03 | Capability spike routing: do **not** expand correlation normalizers on this evidence; Path A judgment **not distinguishable** from `path_a_fact_count` stump (McNemar p=0.625); capability above trivial heuristic remains unmeasured; absolute rates conditional on synthetic 50/50 raw-window corpus, not AlertEnvelope | Live ATLASv2 spike: Path A > Path B on paired anchors (McNemar b=7 c=0 p=0.015625); denser Path B evidence collapses benign specificity; clean negative on primary question is a successful spike | `docs/superpowers/results/2026-08-02-judgment-capability-spike-results.md`; design `docs/superpowers/specs/2026-08-01-capability-spike-design.md` |
...
## DEC-067 — Capability spike routing: coverage not the bottleneck; judgment unmeasured above baseline

**Status:** accepted (2026-08-03)

**Context.** The judgment capability spike (`docs/superpowers/specs/2026-08-01-capability-spike-design.md`) asked whether single-shot GenAI judgment separates malicious from benign ATLASv2 telemetry on Path A (production correlator: Sysmon 1 + Security 4624) vs Path B (generic flattener, all event types). Labels and A≈B thresholds were frozen before provider calls. Full write-up: `docs/superpowers/results/2026-08-02-judgment-capability-spike-results.md`.

**Decisions.**

1. **Do not expand correlation normalizers on this evidence.** Path A outperforms Path B on paired anchor-majority outcomes (McNemar b=7 A✓B✗, c=0 A✗B✓, exact two-sided p=0.015625). Malicious recall is flat across paths; the delta is benign specificity collapsing under richer Path B evidence. Coverage starvation is not the measured bottleneck.

2. **Judgment not shown to beat a trivial heuristic on Path A.** Path A anchor-majority 22/26 vs `path_a_fact_count` stump 20/26; McNemar b=3 c=1, exact two-sided p=0.625. Capability above fact-count remains unmeasured. A clean negative is a successful spike per the design success criterion.

3. **Scope pin for readers of absolute rates.** The corpus is raw telemetry windows at a synthetic 50/50 base rate. Production intake is `AlertEnvelope` (detector-fired, high malicious prior). The A-vs-B comparison cancels that construction error; absolute FP/separation rates do not transfer without an AlertEnvelope-population follow-on (ATLASv2 `cbc-edr-alerts` / `cbc-ngav-alerts`).

**Does not authorize.** Prompt retuning from scored dispositions; promoting the Path B flattener; changing DEC-066 corroboration/enrichment floors; claiming production AlertEnvelope judgment quality.

**Production follow-ups (out of spike scope).** Filed in `memory-bank/tasks.md`: (1) `vertex_provider.py` collapsing `MAX_TOKENS`/`LENGTH` into parse errors; (2) production ingestion paths that can leave seven-fractional-digit RFC3339-ish strings in hashed `normalized_fields` (spike flattener fixed locally; correlators may need the same coerce if they ever emit those EventData keys).

**Evidence.** `evals/capability/captures/atlasv2_capability_spike_results.jsonl` (post-refill); pre-refill backup `*.prerefill.jsonl`; design + results docs above.